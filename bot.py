...
def admin_panel():
    ...
    kb.add(types.InlineKeyboardButton("➕ Канал", callback_data="add_channel"))
    kb.add(types.InlineKeyboardButton("➖ Канал", callback_data="del_channel"))
...
def check_captcha(message):
    if message.text == captcha_answers[message.from_user.id]:
        captcha_answers.pop(message.from_user.id)
        bot.send_message(message.chat.id, "✅ Капча пройдена!", reply_markup=main_menu(message.from_user.id))
    else:
